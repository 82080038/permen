<?php
/**
 * API: Start New Tryout Session - Fixed Version
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id FROM tryout_sessions WHERE user_id = ? AND status = 'ongoing'");
    $stmt->execute([$userId]);
    if ($stmt->fetch()) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT subtes, durasi_menit, jumlah_soal, passing_grade FROM subtes_config WHERE is_active = 1 AND subtes = ?");
        $insertSubtes = $pdo->prepare("INSERT INTO session_subtes (session_id, subtes, waktu_mulai, durasi_menit, jumlah_soal, passing_grade, status) VALUES (?, ?, NOW(), ?, ?, ?, 'ongoing')");
