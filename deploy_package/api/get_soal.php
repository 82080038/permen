...
try {
    ...
    if (!$sessionId) {
        \App\Http\ApiResponse::error('Session ID diperlukan. Pastikan parameter session_id tersedia di URL.', 400);
    }
    if (!$userId) {
        \App\Http\ApiResponse::unauthorized('Autentikasi diperlukan. Silakan login terlebih dahulu.');
    }
    ...
    if (!$session) {
        \App\Http\ApiResponse::forbidden('Session tidak ditemukan atau bukan milik Anda. Periksa session_id yang Anda masukkan.');
    }

    // Cek apakah session masih berjalan
    if ($session['status'] !== 'berjalan') {
        \App\Http\ApiResponse::forbidden('Session sudah selesai atau tidak aktif. Status saat ini: ' . $session['status'] . '.');
    }
    ...
    \App\Http\ApiResponse::success(['session' => $session, 'soal' => $soal, 'passages' => $passages], 'Soal berhasil dimuat');
} catch (PDOException $e) {
    ob_end_clean();
    ...
    \App\Http\ApiResponse::serverError('Database error: ' . $e->getMessage());
} catch (Exception $e) {
    ob_end_clean();
    ...
    \App\Http\ApiResponse::serverError('Terjadi kesalahan server: ' . $e->getMessage());
}
