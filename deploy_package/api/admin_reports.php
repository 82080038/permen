...
if ($action === 'generate_report') {
    ...
    ApiResponse::success(['reports' => $reports], 'Reports retrieved');
    ...
    if (!$scheduleId) {
        ApiResponse::validationError(['schedule_id' => 'Invalid schedule ID'], 'Invalid schedule ID');
    }
    
    $stmt = $pdo->prepare("UPDATE report_schedules SET is_active = ? WHERE id = ?");
    $stmt->execute([$isActive, $scheduleId]);
    
    ApiResponse::success([], 'Schedule updated');
    ...
    ApiResponse::success([], 'Schedule deleted');
