...
require __DIR__ . '/redis_session_handler.php';
...
$redisSessionInitialized = false;
$sessionHandlerUsed = 'files';

if (isRedisAvailable()) {
    try {
        // Try to initialize Redis session handler
        $redisHost = $_ENV['REDIS_HOST'] ?? '127.0.0.1';
        $redisPort = $_ENV['REDIS_PORT'] ?? 6379;
        
        $redisSessionInitialized = initializeRedisSession($redisHost, $redisPort);
        
        if ($redisSessionInitialized) {
            $sessionHandlerUsed = 'redis';
            error_log("[CONFIG] Redis session handler initialized successfully");
        } else {
            error_log("[CONFIG] Redis session handler initialization failed, falling back to files");
        }
    } catch (Exception $e) {
        error_log("[CONFIG] Redis session handler exception: " . $e->getMessage() . ", falling back to files");
    }
} else {
    error_log("[CONFIG] Redis not available, using file sessions");
}
...
error_log("Redis configuration loaded successfully");
?>
