        ...
        if ($noHp && $password) {
            ...
            if ($lockoutStatus['locked']) {
                ...
                if ($user && password_verify($password, $user['password'])) {
                    ...
                    // Add session variables expected by dashboard
                    $_SESSION['nama'] = $user['nama']; // For compatibility
                    $_SESSION['email'] = $user['email'] ?? ''; // For compatibility
                    $_SESSION['role'] = $user['role']; // For compatibility
                    $_SESSION['no_hp'] = $user['no_hp']; // For compatibility
...
<p style="color:#856404;font-size:.75rem;margin-top:.5rem">Admin Password: <code>password</code> | User Password: <code>Sihaloho1982</code></p>
</div>
<script>
