...
$passingMap = $pdo->query("SELECT subtes, passing_grade FROM subtes_config WHERE aktif = 1 LIMIT 3")->fetchAll(PDO::FETCH_KEY_PAIR);
...
async function sendEmailResult(){
    ...
    try {
        const res = await fetch('<?php echo $baseUrl; ?>/api/send_result_notification.php', {
            method: 'POST',
            body: formData
        });
        const data = await res.json();
...
async function loadReview(){
    const res = await fetch('<?php echo $baseUrl; ?>/api/get_review.php?session_id='+reviewSessionId);
    const data = await res.json();
